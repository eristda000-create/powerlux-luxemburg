...
function Test-CentralRuntimeImpactingPath([string]$Path) {
  ...
  foreach ($prefix in $runtimePrefixes) {
    if ($p.Equals($prefix,[StringComparison]::OrdinalIgnoreCase)) { return $true }
  }
  return $false
}

function Invoke-CentralSafeRepoUpdateV2 {
  [CmdletBinding()]
  param(
  ...
  if (-not [string]::Equals($resolvedRepoRoot, $resolvedWorkDir, [StringComparison]::OrdinalIgnoreCase)) { throw 'Safe repo update requires CENTRAL_WORKDIR to be the repository root.' }
  ...
  if ((Normalize-CentralGitRemote $origin) -ne (Normalize-CentralGitRemote $CanonicalRemote)) { throw "Safe repo update refused unexpected origin remote: $origin" }
  ...
  if (-not [string]::IsNullOrWhiteSpace($dirty)) { throw 'Safe repo update requires a completely clean working tree, including no untracked files.' }
  ...
  if ($before -eq $remoteHead) {
    return [ordered]@{ action='git_fast_forward_update'; workdir=$resolvedWorkDir; branch=$branch; origin=$origin; before=$before; after=$before; changed=$false; changed_files=@(); restart_required=$false; status='up_to_date' }
  }

  & git -C $resolvedWorkDir merge-base --is-ancestor $before origin/main 2>$null
  if ($LASTEXITCODE -ne 0) { throw 'Safe repo update refused: local main is not an ancestor of origin/main. No reset/rebase/force operation is permitted.' }
  ...
  $runtimeImpacting = @($changedFiles | Where-Object { Test-CentralRuntimeImpactingPath $_ })
  $restartRequired = ($runtimeImpacting.Count -gt 0)
  ...
  if ($LASTEXITCODE -ne 0 -or $after -ne $remoteHead) { throw "Post-update verification failed. expected=$remoteHead actual=$after" }

  $postDirty = (& git -C $resolvedWorkDir status --porcelain=v1 --untracked-files=all 2>&1 | Out-String).Trim()
  if ($LASTEXITCODE -ne 0 -or -not [string]::IsNullOrWhiteSpace($postDirty)) { throw 'Post-update verification failed: working tree is not clean.' }

  return [ordered]@{
    action='git_fast_forward_update'; workdir=$resolvedWorkDir; branch=$branch; origin=$origin; before=$before; after=$after; changed=$true;
    changed_files=$changedFiles; runtime_impacting_files=$runtimeImpacting; restart_required=$restartRequired; status='fast_forwarded';
    fetch_output=$fetchOutput; merge_output=$mergeOutput
  }
}
