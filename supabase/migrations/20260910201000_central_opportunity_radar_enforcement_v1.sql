-- CENTRAL Opportunity Radar enforcement v1
-- Broad brainstorming jobs must include the canonical Opportunity Radar in Obsidian context.
...
begin
  ...
  v_obsidian_paths := jsonb_build_array('CENTRAL/OPERATING_MODEL.md', v_state_path);
  if v_brainstorm then
    v_obsidian_paths := jsonb_build_array('CENTRAL/OPPORTUNITY_RADAR.md', 'CENTRAL/OPERATING_MODEL.md', v_state_path);
  end if;
  ...
  new.payload := new.payload || jsonb_build_object(
    ...
    'context_budget_policy', case when v_brainstorm then 'opportunity_radar_plus_state_v1' else 'governance_plus_obsidian_state_v1' end,
