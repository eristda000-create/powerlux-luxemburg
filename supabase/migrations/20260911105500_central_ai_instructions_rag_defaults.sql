...
begin
  ...
  v_obsidian_paths := jsonb_build_array('CENTRAL/AI_INSTRUCTIONS.md','CENTRAL/OPERATING_MODEL.md',v_state_path);
  if v_brainstorm then
    v_obsidian_paths := jsonb_build_array('CENTRAL/AI_INSTRUCTIONS.md','CENTRAL/OPPORTUNITY_RADAR.md','CENTRAL/OPERATING_MODEL.md',v_state_path);
  end if;
  ...
  new.payload := new.payload || jsonb_build_object(
    ...
    'knowledge_ai_instructions_path', 'CENTRAL/AI_INSTRUCTIONS.md',
    'knowledge_rag', true,
    ...
    'context_budget_policy', case when v_brainstorm then 'ai_instructions_plus_opportunity_radar_plus_state_v2' else 'ai_instructions_plus_governance_plus_state_v2' end,
    'max_context_chars', case when v_brainstorm then 12000 else 10000 end,
