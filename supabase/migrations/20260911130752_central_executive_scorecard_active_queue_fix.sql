-- Restrict human-decision counts to open states and expose the preferred local execution profile.
...
with streams as (
  ...
  ) order by s.stream_key) v
  from public.central_value_streams s where s.status='active'
), value_totals as (
  ...
  select jsonb_build_object(
    ...
    'human_required_open',count(*) filter(where status in ('reviewing','ready_for_decision','approved') and coalesce((payload->>'requires_human')::boolean,false))
 ...
 select to_jsonb(c) v from public.central_management_coverage_v1 c
), ai as (
 select coalesce(jsonb_agg(to_jsonb(a) order by a.model_key,a.agent_role),'[]'::jsonb) v from public.central_ai_performance_live_v1 a
), human as (
 select count(*)::int v from public.core_engine_work_items
 where status='ready_for_decision'
    or (status in ('reviewing','approved') and coalesce((payload->>'requires_human')::boolean,false))
)
select jsonb_build_object(
  'generated_at',now(),
  'management_kernel_version','v1.2',
  ...
  'needs_human',coalesce((select v from human),0),
  ...
  'recommended_local_execution_profile',jsonb_build_object(
    'default','ollama_prompt_microjob',
    'deep_synthesis','local_agent_team_only_when_needed',
    'reason','Observed 7-day runtime quality favors bounded ollama_prompt jobs over heavy local_agent_team runs.'
  )
);
$$;
